...
//价值链差旅报销审批回写   后面讲tb_total改为tb_Actual_reimbursement_amount 
public class AcceptReimburse4 extends BaseAction {
	...
	public String  execute(RequestInfo request){
		try {
			...
			for (int i = 0; i <properties.length; i++) {
				if("tb_total".equalsIgnoreCase(properties[i].getName())) {
					tb_total=properties[i].getValue();
				}
			...
			if("Y".equals(tb_ims_status)) {
				Logger.log("tb_total > "+tb_total);
				BigDecimal b= new BigDecimal(tb_total);
